...
public class CliSetUserProperties extends AbstractAppApi {
     ...
     *
     */
    private static final String METHOD_SHORT_DESCRIPT = "Sets properties for "
            + "an existing Internal or External User.";
    ...
    */
     ...
     *
     */
    private static final String CLI_SWITCH_REMOVE_CARD = CLI_SWITCH_PFX_REMOVE
            + CLI_OPT_CARD;

    /**
     *
     */
    private static final String CLI_SWITCH_REMOVE_ID = CLI_SWITCH_PFX_REMOVE
            + CLI_OPT_ID;
     ...
     *
     */
    private static final String CLI_SWITCH_REMOVE_PIN = CLI_SWITCH_PFX_REMOVE
            + CLI_OPT_PIN;
    ...
    private static Object[][] theOptions =
            new Object[][] {
                    ...
                    {
                            null,
                            CLI_SWITCH_REMOVE_EMAIL,
                            "Remove Primary Email address (opposed to --"
                                    + CLI_OPT_EMAIL + ")." },
                    {
                            null,
                            CLI_SWITCH_REMOVE_EMAIL_OTHER,
                            "Remove other (secondary) Email addresses (opposed to --"
                                    + CLI_OPT_EMAIL_OTHER + ")." },
                    {
                            null,
                            CLI_SWITCH_REMOVE_CARD,
                            "Remove NFC Card Number (opposed to --"
                                    + CLI_OPT_CARD + ")." },
                    {
                            null,
                            CLI_SWITCH_REMOVE_ID,
    ...
    protected final boolean isValidCliInput(final CommandLine cmd) {
         ...
         */
        if ((cmd.hasOption(CLI_SWITCH_REMOVE_CARD) && cmd
                .hasOption(CLI_OPT_CARD))
                || (cmd.hasOption(CLI_SWITCH_REMOVE_EMAIL) && cmd
                        .hasOption(CLI_OPT_EMAIL))
                || (cmd.hasOption(CLI_SWITCH_REMOVE_EMAIL_OTHER) && cmd
                        .hasOption(CLI_OPT_EMAIL_OTHER))
                || (cmd.hasOption(CLI_SWITCH_REMOVE_ID) && cmd
                        .hasOption(CLI_OPT_ID))
    ...
    protected final AbstractJsonRpcMethodParms createMethodParms(
        ...
        dto.setRemoveEmailOther(this.getSwitchValue(cmd,
                CLI_SWITCH_REMOVE_EMAIL_OTHER));
        dto.setRemoveId(this.getSwitchValue(cmd, CLI_SWITCH_REMOVE_ID));
        dto.setRemovePassword(this.getSwitchValue(cmd,
                CLI_SWITCH_REMOVE_PASSWORD));
